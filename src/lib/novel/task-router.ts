...
export function buildTaskDirective(route: TaskRouteResult): string {
  const directives: Record<NovelTaskIntent, string> = {
    ...
    character_query: "用户在查询人物信息。请先用本地工具（load_context / read_memory 等）查人物状态与认知；若本地无对应实体或信息不足，主动调用 web_search，必要时再 read_web_page，再据此回答。",
    foreshadowing_query: "用户在查询伏笔状态。请列出当前所有伏笔及其状态（已埋设/推进中/已回收）。",
    timeline_query: "用户在查询时间线。请根据时间线数据回答当前时间进展。",
    setting_query: "用户在查询设定信息。请先用本地工具查正史设定与世界观；若本地无对应实体或信息不足，主动调用 web_search，必要时再 read_web_page，再据此回答。",
    general_chat: "",
