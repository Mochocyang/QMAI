...
export function buildStyleExtractionPrompt(sampleText: string, bookTitle: string): string {
  ...
  return [
    ...
    "1. 上述每个维度 key 都要有一个简洁的中文字符串值（1-3 句），描述这本书在该维度上的具体特征，要有可操作性，不要泛泛而谈。",
    "2. `constitution`：把以上维度合成为 5~8 条**可执行的硬约束**（编号列表，写进同一个字符串，用换行分隔），用于约束 AI 之后按这种文风写作。要具体到“怎么写/不怎么写”，例如“环境描写不超过 X 句”“情绪用动作呈现而非总结”。",
    "3. `samples`：从上面提供的原文样本中，**原样摘抄** 3~6 段最能代表这种文风的片段（每段 60~200 字），放进字符串数组。必须是原文照抄，不要改写、不要自己编。",
    "",
    "硬性要求：",
    "- 只分析文风，不要分析人物性格或剧情走向。",
