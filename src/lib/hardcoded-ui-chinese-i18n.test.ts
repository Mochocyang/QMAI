...
describe("user-facing hardcoded chinese strings", () => {
  ...
  it("keeps visible trash and chapter header labels readable when i18n misses keys", () => {
    ...
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.scanStageDetecting", {')
    expect(maintenanceSection).toContain('defaultValue: "正在调用模型分析…"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.processLogTitle", {')
    expect(maintenanceSection).toContain('defaultValue: "分析过程日志"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.mergeLogTitle", {')
    expect(maintenanceSection).toContain('defaultValue: "合并过程日志"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.detectModelLabel", {')
    expect(maintenanceSection).toContain('defaultValue: "检测模型"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.mergeModelLabel", {')
    expect(maintenanceSection).toContain('defaultValue: "合并模型"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.detectModelHint", {')
    expect(maintenanceSection).toContain('defaultValue: "扫描分组用。实体多时上下文更大，建议用更强、上下文更长的模型。"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.mergeModelHint", {')
    expect(maintenanceSection).toContain('defaultValue: "写合并正文用。输入通常更短，可用相对便宜的模型。"')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.selectDetectModel", {')
    expect(maintenanceSection).toContain('t("settings.sections.maintenance.dedup.selectMergeModel", {')
